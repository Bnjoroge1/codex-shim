...
def write_config(models: list[VibeProxyModel], path: Path, catalog_path: Path, base_url: str, provider_name: str | None = None) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
...
def codex_config_overrides(catalog_path: Path, default_slug: str, base_url: str, provider_name: str | None = None) -> list[str]:
    name = provider_name or default_slug
    return [
        f'model="{_toml_escape(default_slug)}"',
        f'model_provider="{PROVIDER_NAME}"',
        f'model_catalog_json="{_toml_escape(str(catalog_path))}"',
        f'model_providers.{PROVIDER_NAME}.name="{_toml_escape(name)}"',
